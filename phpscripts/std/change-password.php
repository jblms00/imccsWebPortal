...
if ($_SERVER['REQUEST_METHOD'] == "POST") {
    ...
    if (empty($old_password)) {
    ...
    } else if ($user_id !== $user_data['student_id']) {
        $data['status'] = "error";
    ...
    } else if ($old_password !== base64_decode($user_data["student_password"])) {
        $data["status"] = "error";
        ...
        $update_query = "UPDATE student_data SET student_password = '$new_encoded_password' WHERE student_id = '$user_id'";
